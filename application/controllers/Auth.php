...
class Auth extends CI_Controller
{
    ...
    /**
    * Fungsi _login melakukan pengecekan apakah user telah terdaftar di database atau tidak
    * 

    */
    private function _login()
    {
        // mengambil value username pada kolom yang telah diisi pada form login dan disimpan dalam variabel username
        $username = $this->input->post('username'); 

        // mengambil value password pada kolom yang telah diisi pada form login dan disimpan dalam variabel password
        $password = $this->input->post('password');
        // model
        //Memanggil model User_model sebagai user
        $this->load->model('User_model', 'user');

        //memanggil fungsi userCheckLogin dari model dengan menggunakan parameter berupa variabel username
        $user = $this->user->userCheckLogin($username);

        /*Jika user telah terdaftar dan ada pada database maka akan dilakukan pengecekan nilai active
         * Jika nilai active =1 maka akan dilakukan pengecekan password dan apabila password sama maka data akan diambil untuk digunakan sebagai user yang sedang aktif
         *Selanjutnya tipe user akan dicek apabila user memiliki tipe 1 = admin maka user akan diarahkan ke menu admin
         * Jika tipe user bernilai 2 = polisi maka user akan diarahkan ke menu polisi
         * Jika tipe user bernilai 3 = pelanggar maka user akan diarahkan ke menu pelanggar 
        */
        if ($user != null) {
            if ($user['active'] == 1) {
                ...
                } else { //Jika password salah maka akan ditampilkan pesan "Wrong Password!" dan user akan diarahkan kembali ke halaman login
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Wrong password!</div>');
                    redirect('auth');
                }
            } else { //Jika nilai active bukan 1 maka akan ditampilkan pesan "Akun anda tidak aktif" dan user akan diarahkan kembali ke halaman login
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Akun anda tidak aktif!</div>');
                redirect('auth');
            }
        } else { //Jika username tidak ditemukan di database maka akan ditampilkan pesan "Email is not registered" dan user akan diarahkan kembali ke halaman 
